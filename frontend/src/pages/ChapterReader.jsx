...
/* ═══════════════════════════════════════════
   Manga Page with Comments – icon on image,
   comment panel slides in BESIDE the image
   ═══════════════════════════════════════════ */
function MangaPageWithComments({ page, idx, totalPages, storyId, chapterId, user }) {
  ...
  useEffect(() => {
    getCommentsByPage(chapterId, idx)
  ...
  const submit = async () => {
    ...
    if (!text.trim()) return;
    try {
      await createComment({ storyId, chapterId, pageIndex: idx, content: text });
      setText('');
      await load();
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', flex: '1 1 auto', minWidth: 0, maxWidth: '900px' }}>
        {/* Page number label */}
        <div style={{
          position: 'absolute',
          top: '8px',
      ...
      {/* ── Right: Comment panel beside the image ── */}
      <div
        ref={panelRef}
        style={{
          width: open ? '340px' : '0px',
          minWidth: open ? '340px' : '0px',
          maxHeight: '600px',
          opacity: open ? 1 : 0,
          overflow: 'hidden',
          ...
          <div style={{
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '0.6rem 0.85rem' }}>
              {loading ? (
                ...
                <p style={{ color: '#64748b', fontSize: '0.8rem', textAlign: 'center', margin: '1.5rem 0' }}>Chưa có bình luận nào.</p>
              ) : (
                comments.map((c) => (
                  <div key={c.id} style={{
                    padding: '0.5rem 0',
...
export default function ChapterReader() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: isManga ? 'var(--bg-primary)' : bgColor, transition: 'background 0.25s ease' }}>
      ...
      {/* Content */}
      <div style={{ maxWidth: isManga ? undefined : '750px', margin: '0 auto', padding: '1rem' }}>
        {isManga ? (
          /* === MANGA READER: Image Pages with side comment panel === */
